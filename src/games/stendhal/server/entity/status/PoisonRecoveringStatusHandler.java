...
public class PoisonRecoveringStatusHandler implements StatusHandler <PoisonRecoveringStatus>{
	 ...
	 */
	@Override
	public void inflict(PoisonRecoveringStatus status, StatusList statusList, Entity attacker) {
		...
		if (count == 0) {
			...
			TurnNotifier.get().notifyInTurns(0, new PoisonRecoveringStatusTurnListener(statusList));
		}
	}
	 ...
	 */
	@Override
	public void remove(PoisonRecoveringStatus status, StatusList statusList) {
		...
		Status nextStatus = statusList.getFirstStatusByClass(PoisonRecoveringStatus.class);
		if (nextStatus != null) {
			...
			if (entity.has("poisoned")) {
				...
				entity.getStatusList().setImmune(StatusType.POISONED);
			}
		}
	}
}
